...
public abstract class regras {
    ...
    public boolean atribuirValor(String value,char parameter){
        if(value.equalsIgnoreCase("proximo")) return true;
        try {
            Double.parseDouble(value);
        } catch (Exception e) {
            System.out.print("o valor de ("+ parameter+") é invalido por favor digite um numero!\n"+ "Digite o valor de ("+ parameter + ") ou 'proximo' para digitar o proximo parametro ou encerrar: ");
            return false;
        }
        this.value = value;
        System.out.print("Digite o valor de ("+ parameter + ") ou 'proximo' para digitar o proximo parametro ou encerrar: ");
        return false;
    }
